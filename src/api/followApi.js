 ...
 * 41、21 分享视频
 */
export function getUserFollowerData_api(uid, pagesize, lasttime) {
  if (lasttime <= 0) {
    return axios.get(
      `/user/event?uid=${uid}&limit=${pagesize}&timestamp=${Date.now()}`
    );
  }
  return axios.get(
    `/user/event?uid=${uid}&limit=${pagesize}&lasttime=${lasttime}&timestamp=${Date.now()}`
  );
}
